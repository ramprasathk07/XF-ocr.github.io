...
const SecureFilePreview = ({ src, type, className }: { src: string, type: 'pdf' | 'image', className?: string }) => {
  ...
  useEffect(() => {
    ...
    const loadFile = async () => {
      try {
        ...
        console.error("Failed to load secure file preview", err);
        setLoading(false);
      }
    };
  ...
  if (!blobUrl) return <div className="error-msg">Failed to load preview</div>;

  return type === 'pdf' ? (
    <iframe className={className} src={blobUrl} />
  ) : (
    <img className={className} src={blobUrl} alt="Preview" />
  );
};

export default function Dashboard() {
  const [currentUser, setCurrentUser] = useState<User | null>(null);
  const [currentModel, setCurrentModel] = useState("xf1-mini");
                ...
                <div className="pane-content">
                  {selectedHistory.savedFiles[0]?.type === 'pdf' ? (
                    <SecureFilePreview
                      ...
                      src={`${API_BASE}${selectedHistory.savedFiles[0].saved_path}`}
                      type="pdf"
                    />
                  ) : (
