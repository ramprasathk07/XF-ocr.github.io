...
# Initialize Logger
logger = setup_logger(name="pdf_renderer", log_dir="logs")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Production-ready PDF Render Script")
    ...
    parser.add_argument("--workers", type=int, default=max(1, (os.cpu_count() or 2) // 2), help="Number of worker threads.")
    parser.add_argument("--dpi", type=int, default=240, help="DPI resolution for rendering.")
...
def process_pdf_and_return_paths(pdf_path, output_dir, workers=4, dpi=240) -> List[str]:
    # Wrapper for external calls vs CLI usage
    # Refactor process_pdf to return the list
    return process_pdf(pdf_path, output_dir, workers, dpi)
